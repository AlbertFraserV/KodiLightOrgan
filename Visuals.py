...
class Visuals:
    ...
    def play(self):
        ...
        freq_ranges = np.arange(100, int(self.audio.frequencies.max()), len(self.audio.frequencies))
        while running:
            screen.fill((0,0,0))
            pygame.display.flip()
            t = pygame.time.get_ticks()
            ...
            for f in freq_ranges:
                ...
                self.draw_rect(screen, (random.int(0,254),random.int(0,254),random.int(0,254)), (10,10), (x,y), self.calc_alpha(self.max_db, db))
                x += 10; y += 10
                rects.append(db)
    ...
    def draw_rect(self, screen, color, size, location, alpha):
        # rect = Rect(location[0],location[1],size[0],size[1])
        # rect.center = ((location[0], location[1]))
